...
# Do not run oc/kubectl here: linux/amd64 image builds on arm64 (QEMU) often SIGSEGV
# when executing glibc-linked clients (exit 139). Runtime on real amd64 is unaffected.
...
RUN GEN=/cachi2/output/deps/generic \
    && if [ -f "${GEN}/helm-linux-amd64.tar.gz" ]; then \
         cp "${GEN}/helm-linux-amd64.tar.gz" /tmp/helm.tar.gz; \
       else \
         curl -fsSL "https://get.helm.sh/helm-v4.1.4-linux-amd64.tar.gz" -o /tmp/helm.tar.gz; \
       fi \
